...
from dsp_be.routes.test.stars_test import TEST_STAR, create_star, delete_star
...
async def create_planet(
    ...
    request = {"name": planet_name, "star_name": star_name, "resources": resources_dict, "imports": imports_list, "exports": exports_list}
    return await client.post("/dsp/api/planets/", json=request)


async def read_planet(client: AsyncClient, star_name: str, planet_name: str) -> Response:
...
async def update_planet(
    ...
    exports_list = exports if exports is not None else []
    resources_dict = resources if resources is not None else {}
    request = {"id": id_, "name": name, "star_name": star_name, "resources": resources_dict, "imports": imports_list, "exports": exports_list}
    return await client.put("/dsp/api/planets/", json=request)


async def delete_planet(client: AsyncClient, star_name: str, planet_name: str) -> Optional[Response]:
...
async def test_create_planet(async_client: AsyncClient) -> None:
    ...
    response = await create_planet(async_client, star_name=TEST_STAR, planet_name=TEST_PLANET)
    assert response.status_code == 200
    response = await read_planet(async_client, star_name=TEST_STAR, planet_name=TEST_PLANET)
    assert response.status_code == 200
